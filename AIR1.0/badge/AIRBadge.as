...
package {
	...
	public class AIRBadge extends MovieClip {
		...
		private var CHS:Boolean=false
		public function AIRBadge() {
			language=Capabilities.language;
			CHS=("zh-CN"==language)
			
			root.button_chs.visible=false;
			if(CHS){root.button_chs.visible=true;root.button.visible=false;}
			// Read FlashVars
			...
			switch (_air.getStatus()) {
				...
				case "available" :
					if (_appName && _appName.length > 0) {
						if (CHS)
							root.statusMessage.htmlText = "<p align='center'><font size='12' color='#" + _messageColor + "'>为了运行 " + _appName + "，安装程序会同时安装 Adobe® AIR™.</font></p>";
						else
							root.statusMessage.htmlText = "<p align='center'><font size='10' color='#" + _messageColor + "'>In order to run " + _appName + ", this installer will also setup Adobe® AIR™.</font></p>";
					} else {
						if (CHS)
							root.statusMessage.htmlText = "<p align='center'><font size='12' color='#" + _messageColor + "'>为了运行这个应用程序，安装程序会同时安装 Adobe® AIR™ 。</font></p>";
						else
							root.statusMessage.htmlText = "<p align='center'><font size='10' color='#" + _messageColor + "'>In order to run this application, this installer will also setup Adobe® AIR™.</font></p>";
					}
					break;
				case "unavailable" :
					if (CHS)
						root.statusMessage.htmlText = "<p align='center'><font size='12' color='#" + _messageColor + "'>您的系统上尚未安装 Adobe® AIR™ 。</font></p>";
					else
						root.statusMessage.htmlText = "<p align='center'><font size='10' color='#" + _messageColor + "'>Adobe® AIR™ is not available for your system.</font></p>";
		...
		private function onButtonClicked(e:Event):void {
			try {
				switch (_air.getStatus()) {
					case "installed" :
						if (CHS)
							root.statusMessage.htmlText = "<p align='center'><font size='12' color='#" + _messageColor + "'>下载并打开AIR文件进行安装。</font></p>";
						else
							root.statusMessage.htmlText = "<p align='center'><font size='10' color='#" + _messageColor + "'>Download and open the AIR file to begin the installation.</font></p>";
						_air.installApplication( _appURL, _airVersion );
						break;
					case "available" :
						if (CHS)
							root.statusMessage.htmlText = "<p align='center'><font size='12' color='#" + _messageColor + "'>开始安装...</font></p>";
						else
							root.statusMessage.htmlText = "<p align='center'><font size='10' color='#" + _messageColor + "'>Starting install...</font></p>";
						_air.installApplication( _appURL, _airVersion );
						break;
					case "unavailable" :
						// do nothing
